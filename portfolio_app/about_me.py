...
def about_me(about_me_data, source_path):
    ...
    with col3:
        if "resume_en" in about_me_data:
            resume = source_path / Path(about_me_data["resume_en"])
        ...
        if "resume_pt" in about_me_data:
            resume = source_path / Path(about_me_data["resume_pt"])
            with open(resume, "rb") as f:
                bytes_data = f.read()
            st.download_button(
